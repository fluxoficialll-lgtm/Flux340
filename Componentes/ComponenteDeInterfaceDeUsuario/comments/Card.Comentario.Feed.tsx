...
interface PainelComentariosFeedProps {
    isOpen: boolean;
    onClose: () => void;
    postAuthor: string;
    comments: Comment[];
...
export const PainelComentariosFeed: React.FC<PainelComentariosFeedProps> = ({
    isOpen, onClose, postAuthor, comments, commentText, onCommentTextChange, onSend,
    onLike, onDelete, onUserClick, currentUserId, replyingTo, onCancelReply, onReplyClick, currentUserAvatar
}) => {
    const inputRef = useRef<HTMLInputElement>(null);
    const placeholder = `Adicionar um comentário como @${currentUserId}...`;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 animate-fade-in-fast flex items-end" onClick={onClose}>
            ...
            <div className="bg-[#101010] w-full max-w-2xl mx-auto h-[85dvh] rounded-t-2xl flex flex-col comment-panel-animate" onClick={(e) => e.stopPropagation()}>
                <div className="text-center py-4 border-b border-white/10 relative">
                    <h3 className="font-bold text-sm">Comentários de {postAuthor}</h3>
